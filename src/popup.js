...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function addSessionToUI(session) {
    ...
    // Create the main session info and action buttons with monochrome icons
    sessionElement.innerHTML = `
      <div class="session-info">
        <div class="title-container">
          <span class="session-title" data-session-id="${session.id}" contenteditable="true">${session.name}</span>
          <span class="session-date">${formatDate(session.create_time)}</span>
        </div>
        <div class="session-actions">
          <button class="show-tabs-btn" data-session-id="${session.id}" title="Show Tabs">▼</button>
          <button class="restore-btn" data-session-id="${session.id}" title="Open Here">⊕</button>
          <button class="new-window-btn" data-session-id="${session.id}" title="Open in New Window">⟴</button>
          <button class="delete-btn" data-session-id="${session.id}" title="Move to Trash">🗑︎</button>
        </div>
      </div>
    `;
    ...
    dateElement.textContent = `Created: ${formatDate(session.create_time)}`;
        ...
        sessionElement.innerHTML = `
          ...
          formatDate(session.create_time)
            ...
            <div class="session-actions">
              <button class="show-tabs-btn" data-session-id="${session.id}" title="Show Tabs">▼</button>
              <button class="restore-btn" data-session-id="${session.id}" title="Open Here">⊕</button>
              <button class="new-window-btn" data-session-id="${session.id}" title="New Window">⟴</button>
              <button class="delete-btn" data-session-id="${session.id}" title="Move to Trash">🗑︎</button>
            </div>
          </div>
        `;
      ...
      trashSessions.forEach((session) => {
        ...
        sessionElement.innerHTML = `
          ...
          formatDate(session.create_time)
            ...
            <div class="session-actions">
              <button class="show-tabs-btn" data-session-id="${session.id}" title="Show Tabs">▼</button>
              <button class="undelete-btn" data-session-id="${session.id}" title="Restore">♻︎</button>
              <button class="delete-forever-btn" data-session-id="${session.id}" title="Delete Forever">🗑︎</button>
            </div>
          </div>
        `;
